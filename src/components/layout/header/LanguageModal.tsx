...
const LanguageSelect = ({ textColor = "text-white" }: LanguageSelectProps) => {
	...
	return (
		<div className="relative inline-block" ref={menuRef}>
			...
			{isOpen && (
				<div className="absolute left-0 mt-2 w-24 bg-[#ffffff]  text-black border-gray-200 rounded-lg shadow-lg z-10 overflow-hidden animate-fadeIn">
					{options.map((option) => (
						<div
							key={option.value}
							className="px-4 py-2 text-sm hover:bg-[#c9c9c9] hover:text-black cursor-pointer transition-all flex items-center gap-2"
							onClick={() => {
